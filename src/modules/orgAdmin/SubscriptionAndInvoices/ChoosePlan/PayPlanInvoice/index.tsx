...
const PayPlanInvoice: FC<PayInvoiceI> = ({ open, onClose, invoiceId }) => {
  ...
  const { data, isLoading: invoiceIsLoading } = useGetSingleInvoicesByIdQuery(
    { id: invoiceId },
  ...
  return (
    <CommonDrawer
      ...
      isDisabled={data?.data?.status === 'PAID'}
    >
      {invoiceIsLoading ? (
        <Box sx={{ width: '100%' }}>
          <LinearProgress />
        ...
        <>
          ...
          <Box sx={{ mt: '8px' }}>
            <Box sx={styles?.iRow}>
              <Box sx={styles?.iCell}>
                ...
                {data?.data?.details?.calculations?.perDayPerStoragePrice} per
                day)
              </Box>
              <Box sx={styles?.iCellHead}>
          ...
          {/* {data?.data?.details?.calculations?.planDiscountAmount > 0 &&  } */}
          <Box sx={{ mt: '8px' }}>
            <Box sx={styles?.iRow}>
              <Box sx={styles?.iCell}>
                Plan Discount (
              ...
              <Box sx={styles?.iCellHead}>
                £{' '}
                {data?.data?.details?.calculations?.planDiscountAmount?.toFixed(
                  2,
                )}
              </Box>
            </Box>
          ...
          {/* {data?.data?.details?.calculations?.invoiceDiscountAmount > 0 && } */}
          <Box sx={{ mt: '8px' }}>
            <Box sx={styles?.iRow}>
              <Box sx={styles?.iCell}>
                Invoice Discount (
          ...
          {/* {data?.data?.details?.calculations?.alreadyPaid > 0 && } */}
          <Box sx={{ mt: '8px' }}>
            <Box sx={styles?.iRow}>
