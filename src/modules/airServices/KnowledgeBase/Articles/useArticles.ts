...
import { useDeleteArticleMutation, useGetArticlesQuery, useGetFoldersQuery } from '@/services/airServices/assets/knowledge-base/articles';
...
export const useArticles = () => {
  ...
  const { data, isLoading, isSuccess, isError, isFetching }: any = useGetArticlesQuery(queryParams);
  const articlesData = data?.data?.articles?.map((article: any) => ({
    id: article?._id ?? "---",
  ...
  const foldersList = [{ folderName: "all", _id: "all" }, ...folderData?.data ?? []];

  const handleSelectedArticlesTab = (tab: string) => {
    if (tab !== "all") {
      setQueryParams((prev: any) => ({ ...prev, folderId: tab }))
    } else {
      setQueryParams(({ folderId, ...rest }: any) => ({ ...rest }))
    }
