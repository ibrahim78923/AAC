...
export const totalSeriesBar = (pipelineForecastData: any) => {
  return pipelineForecastData?.stages?.flatMap(
    (pipeline: any) =>
      pipeline?.stages?.map((stage: any) => {
        const graphData = pipelineForecastData?.userStats?.find(
          (g: any) =>
            g?.stageGroups?.some((sg: any) => sg?.stageId === stage?._id),
        );
...
export const totalOptionsBar: any = (theme: any, pipelineForecastData: any) => {
  ...
  const numberOfStages = pipelineForecastData?.stageGroups?.length || 0;
