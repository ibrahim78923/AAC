...
const SendEmailDrawer = (props: any) => {
  const { openDrawer, setOpenDrawer, drawerType, emailSettingsData, dealId } =
    props;
  const {
    handleSubmit,
    onSubmit,
  ...
  const { outlookStates, gmailStates } = useEmails();
  ...
  const [isGmailAuthErr, setIsGmailAuthErr] = useState(false);
  const [isOutlookAuthErr, setIsOutlookAuthErr] = useState(false);

  const handelIsGmailAuthenticated = () => {
    if (gmailStates?.gmailFoldersData?.data?.labels?.length) {
  ...
  const handelIsOutlookAuthenticated = () => {
    if (outlookStates?.outlookFoldersData?.data?.folders?.length) {
        ...
        {valueProvider.length ? (
          ...
          <>
            ...
            >
              ...
              >
                {outlookStates?.outlookFoldersLoading ||
                gmailStates?.gmailFoldersLoading ? (
                  <Box
                    sx={{
                      gap: 1,
                      display: 'flex',
              ...
              {isGmailAuthErr && (
                <Box sx={{ textAlign: 'center' }}>
                  You are not authenticated as <strong>Gmail</strong> <br />
                  <span
              ...
              {isOutlookAuthErr && (
                <Box sx={{ textAlign: 'center' }}>
                  You are not authenticated as <strong>Outlook</strong> <br />
                  <span
