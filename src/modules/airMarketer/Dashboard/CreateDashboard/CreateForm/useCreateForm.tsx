...
const useCreateForm = (formType: any) => {
  ...
  const disableAccess =
    currentUser === getMarketingDashboardById?.data?.createdBy ||
    formType === DRAWER_TYPES?.ADD
      ? false
      : true;

  const [updateMarketingDashboard, { isLoading: loadingUpdateDashboard }] =
    useUpdateMarketingDashboardMutation();
  ...
  useEffect(() => {
    if (selectedDashboardId) {
      const data = getMarketingDashboardById?.data;
  ...
  const onSubmit = async (values: any) => {
    ...
    try {
      if (formType === DRAWER_TYPES?.EDIT) {
        await updateMarketingDashboard({
          body: { id: selectedDashboardId, ...payload },
        })?.unwrap();
      } else {
        await postMarketingDashboard({
          body: payload,
        })?.unwrap();
      }
      reset();
